...
create_tax_annot <- function(ps, annotation_file, level="Genus"){
  ...
  for (otu in otu_all) {
    ...
    writeLines(paste(Class,"annotation_background_color", color, sep = "\t"), file_conn)
    # writeLines(paste(class, "annotation_rotation","90", sep = "\t"), file_conn)
    writeLines(paste(Class,"clade_marker_size", "90", sep = "\t"), file_conn)
    writeLines(paste(Class,"clade_marker_color", color, sep = "\t"), file_conn)
    # writeLines(paste(genus,"annotation", genus, sep = "\t"), file_conn)
    writeLines(paste(Genus,"annotation_background_color", color, sep = "\t"), file_conn)
    # writeLines(paste(genus, "annotation_rotation","90", sep = "\t"), file_conn)
    writeLines(paste(Genus,"clade_marker_size", "60", sep = "\t"), file_conn)
    writeLines(paste(Genus,"clade_marker_color", color, sep = "\t"), file_conn)
